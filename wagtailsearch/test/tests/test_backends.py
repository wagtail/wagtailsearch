...
from wagtailsearch.backends import (
    InvalidSearchBackendError,
    get_search_backend,
    get_search_backends,
)
from wagtailsearch.backends.base import BaseSearchBackend, FieldError, FilterFieldError
...
from wagtailsearch.query import (
    MATCH_ALL,
    MATCH_NONE,
    And,
    Boost,
...
class TestBackendLoader(TestCase):
    ...
    def test_import_by_name_postgres_db_vendor(self):
        from wagtailsearch.backends.database.postgres.postgres import (
    ...
    def test_import_by_name_sqlite_db_vendor(self):
        ...
        if not fts5_available():
            ...
            from wagtailsearch.backends.database.sqlite.sqlite import (
                SQLiteSearchBackend,
            )
    ...
    def test_nonexistent_backend_import(self):
        self.assertRaises(
            InvalidSearchBackendError,
            get_search_backend,
            backend="wagtailsearch.backends.doesntexist",
        )
    ...
    @override_settings(
        WAGTAILSEARCH_BACKENDS={
            "default": {"BACKEND": "wagtailsearch.backends.database"},
            "another-backend": {"BACKEND": "wagtailsearch.backends.database"},
        }
    )
