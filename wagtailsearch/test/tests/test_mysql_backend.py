...
@override_settings(
    WAGTAILSEARCH_BACKENDS={
        "default": {
            "BACKEND": "wagtailsearch.backends.database.mysql.mysql",
        }
    }
)
class TestMySQLSearchBackend(BackendTests, TransactionTestCase):
    backend_path = "wagtailsearch.backends.database.mysql.mysql"
    ...
    def test_lexeme_validation(self):
        from wagtailsearch.backends.database.mysql.query import Lexeme
    ...
    def test_get_search_field_for_related_fields(self):
        """
        The get_search_field method of MySQLSearchQueryCompiler supports retrieving search fields
