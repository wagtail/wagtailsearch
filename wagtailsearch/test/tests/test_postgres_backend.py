...
@override_settings(
    WAGTAILSEARCH_BACKENDS={
        "default": {
            "BACKEND": "wagtailsearch.backends.database.postgres.postgres",
        }
    }
)
class TestPostgresSearchBackend(BackendTests, TestCase):
    backend_path = "wagtailsearch.backends.database.postgres.postgres"

    def test_weights(self):
        from wagtailsearch.backends.database.postgres.weights import (
            BOOSTS_WEIGHTS,
            WEIGHTS_VALUES,
    ...
    def test_get_search_field_for_related_fields(self):
        """
        The get_search_field method of PostgresSearchQueryCompiler supports retrieving search fields
